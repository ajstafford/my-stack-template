...
async function main() {
  console.clear()
  log('\n🚀 Welcome to My Stack Template Setup!\n', colors.bright + colors.blue)

  // Check if .env.local already exists
  const envExists = await fileExists('.env.local')
  if (envExists) {
    log('⚠️  .env.local already exists!', colors.yellow)
  ...
  if (hasProject.toLowerCase() !== 'y') {
    log('\n📝 Let\'s create a Supabase project first!\n', colors.blue)
  ...
  while (!supabaseUrl) {
    ...
    if (!supabaseUrl) {
      ...
      log('❌ URL should look like: https://xxxxx.supabase.co\n', colors.red)
      supabaseUrl = ''
    }
  }
  ...
  while (!supabaseAnonKey) {
    ...
    if (!supabaseAnonKey) {
      log('❌ Anon Key cannot be empty!\n', colors.red)
    }
  }

  // Verify connection
  log('\n🔍 Verifying connection...', colors.blue)
  const isValid = await verifySupabaseConnection(supabaseUrl, supabaseAnonKey)

  if (!isValid) {
    log('⚠️  Could not verify connection to Supabase.', colors.yellow)
    ...
    log('✅ Connection verified!', colors.green)
  }
...
`
  ...
  try {
    ...
    log('\n✅ .env.local file created successfully!', colors.green)
  } catch (error) {
    log('\n❌ Error creating .env.local file:', colors.red)
  ...
  log('💡 Tip: Check the README.md for more information.\n')
